...
void play() {
  ...
  while(!is_game_over())
  {
    ...
    if(num_pawns_in_play > 1 || dice_roll == 6)
    {
...
void turn_on_players_finish() {
  CRGB color_led = Yellow1;
  for(int i = 0; i < 8; i++)
  {
    leds[2][i] = Yellow1;
    leds[2][i + 8] = CRGB::Red;
    leds[2][i + 16] = CRGB::Green;
    leds[2][i + 24] = CRGB::Blue;
    FastLED.show();
    delay(60);
  }
  ...
  {
    leds[2][i] = CRGB::Black;
    leds[2][i + 8] = CRGB::Black;
    leds[2][i + 16] = CRGB::Black;
    leds[2][i + 24] = CRGB::Black;
    FastLED.show();
    delay(60);
  }
  delay(1000);
...
void blink_players_space(int player) {
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    else if(players[player].pawns[i].status == (Status)FINISH)
    {
      leds[2][curr_pos] = CRGB::Black;
    }
    else
...
int get_next_pawn(int prev_pawn_index, int player_index, int dice_num) {
  ...
  do
  {
    ...
    {
      if((players[player_index].pawns[curr].position + dice_num) <= (get_final_index_led(player_index) - 1))
        return curr;
    }
...
void blink_pawn(int pawn_index, int player_index)
{
  ...
  int status_pawn = players[player_index].pawns[pawn_index].status;
  ...
  if(status_pawn == (Status)FINISH)
    leds[2][pos] = CRGB::Black;
  else if(pos != -1)
...
int get_final_index_led(int current_player) {
  if(current_player == 0) // red
    return 8;
  else if(current_player == 1) // green
    return 16;
...
void move_pawn(int current_player, int pawn_index, int spaces) {
  ...
  if(pawn.status == (Status)FINISH)
  {
    blink_moving_to_final(current_player, pawn_index, current_pos, spaces, 1);//players[current_player].pawns[pawn_index].position = 
    return;
  }
  ...
  if(current_pos <= get_final_index(current_player) && new_pos > get_final_index(current_player)) { //final
    ...
    blink_moving_to_final(current_player, pawn_index, current_pos, spaces, 0);
    return;
  }

  if(current_pos != get_start_index(current_player))
  {
...
void blink_moving_to_final(int player_index, int pawn_index, int position, int spaces, bool status_finished) {
  CRGB crgb_color;
  if((Color)player_index == (Color)RED)
    crgb_color = CRGB::Red;
  else if((Color)player_index == (Color)GREEN)
  ...
  {
    new_spaces = get_final_index(player_index) - position;
    if(new_spaces == 0)
      new_spaces = spaces;
    else
    {
      if(leds[0][get_final_index(player_index)])
      {
        color_there = leds[0][get_final_index(player_index)];
        color_th = 1;
      }
      blink_moving(player_index, position, get_final_index(player_index) - position);
    }
  ...
  if(color_th)
  {
    leds[0][get_final_index(player_index)] = color_there;
  }
  else
  {
    leds[0][get_final_index(player_index)] = CRGB::Black;
  }
  FastLED.show();
  delay(70);
  ...
  if(status_finished == 0)
  {
    leds[2][(((player_index + 1) % 4) * 8) + (new_spaces - 1)] = crgb_color;
    players[player_index].pawns[pawn_index].position = (((player_index + 1) % 4) * 8) + (new_spaces - 1);
  }
  else
  {
    leds[2][position + new_spaces] = crgb_color;
    players[player_index].pawns[pawn_index].position = position + new_spaces;
  }
